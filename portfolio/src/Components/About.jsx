...
const projectCards = [
  {
    ...
    chip: 'lms-e-learning-system.vercel.app',
    description:
    ...
    tech: [
      { label: 'Next.js', icon: <SiNextdotjs className="w-4 h-4" />, color: 'text-white' },
      ...
      { label: 'Socket.io', icon: <SiSocketdotio className="w-4 h-4" />, color: 'text-white' },
  ...
  },
  {
    ...
    tech: [
      { label: 'Next.js 14', icon: <SiNextdotjs className="w-4 h-4" />, color: 'text-white' },
      { label: 'Node.js', icon: <SiNodedotjs className="w-4 h-4" />, color: 'text-[#339933]' },
      { label: 'Express', icon: <SiExpress className="w-4 h-4" />, color: 'text-gray-200' },
      { label: 'MongoDB', icon: <SiMongodb className="w-4 h-4" />, color: 'text-[#47A248]' },
      ...
      { label: 'Tailwind CSS', icon: <SiTailwindcss className="w-4 h-4" />, color: 'text-[#06B6D4]' },
      { label: 'Shadcn/UI', icon: <SiShadcnui className="w-4 h-4" />, color: 'text-white' },
    ],
  },
];
      ...
      <section id="about" className="py-14 md:py-24 relative z-10">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
          <motion.div
            ...
            className="space-y-6"
          >
            <motion.div variants={sectionVariants} className="w-full border border-white/10 bg-[#101010] p-6 md:p-8">
              <div className="flex items-center justify-between mb-5">
                <span className="text-xs md:text-sm font-semibold tracking-widest text-white/80 uppercase">About Me</span>
                <span className="text-[10px] md:text-xs font-medium tracking-widest text-white/40 uppercase">Profile</span>
              </div>

              <div className="space-y-4 text-sm md:text-base leading-relaxed text-white/75">
            ...
            {projectCards.map((project) => (
              <motion.div key={project.number} variants={sectionVariants} className="w-full border border-white/10 bg-[#101010] p-6 md:p-8">
                <div className="text-xs text-white/40 mb-3 mono">{project.number}</div>

                <a
                  href={project.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 text-xl md:text-2xl font-bold text-white hover:text-white/85 transition-colors duration-200"
                >
                  <span>{project.name}</span>
                  <ExternalLink className="w-4 h-4" />
                </a>

                <p className="mt-2 text-sm md:text-base text-white/55">{project.subtitle}</p>

                <a
                  href={project.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-block mt-4 mono text-xs text-white/80 border border-white/15 bg-white/5 px-3 py-1.5 hover:bg-white/10 transition-colors duration-200"
                >
                  {project.chip}
                </a>

                <p className="mt-5 text-sm md:text-base leading-relaxed text-white/70">{project.description}</p>

                <div className="mt-5 flex flex-wrap gap-2">
                  {project.tech.map((item) => (
                    <span key={item.label} className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md border border-white/15 bg-white/5 text-xs text-white/80">
            ...
            <motion.div variants={sectionVariants} className="w-full border border-white/10 bg-[#101010] p-6 md:p-8">
              <h3 className="text-xs md:text-sm font-semibold tracking-widest text-white/80 uppercase mb-5">Tech Stack</h3>
